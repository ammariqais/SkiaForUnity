...
namespace Svg.Model.Drawables {
    ...
    public abstract class DrawableContainer : DrawableBase {
        ...
        protected void CreateChildren(SvgElement svgElement, SKRect skViewport, DrawableBase? parent,
            IAssetLoader assetLoader, HashSet<Uri>? references, DrawAttributes ignoreAttributes) {
            foreach (var child in svgElement.Children) {
                var drawable =
                    DrawableFactory.Create(child, skViewport, parent, assetLoader, references, ignoreAttributes);
                if (drawable is { }) {
