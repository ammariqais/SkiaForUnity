...
namespace Svg.Model.Drawables.Elements {
    ...
    public sealed class PolylineDrawable : DrawablePath {
        ...
        public static PolylineDrawable Create(SvgPolyline svgPolyline, SKRect skViewport, DrawableBase? parent,
            ...
            drawable.IsDrawable = drawable.CanDraw(svgPolyline, drawable.IgnoreAttributes) &&
                                  drawable.HasFeatures(svgPolyline, drawable.IgnoreAttributes);
        ...
        private void Initialize(SKRect skViewport, HashSet<Uri>? references) {
            ...
            if (SvgExtensions.IsValidFill(svgPolyline)) {
                Fill = SvgExtensions.GetFillPaint(svgPolyline, GeometryBounds, AssetLoader, references,
                    IgnoreAttributes);
                if (Fill is null) {
                    canDrawFill = false;
                }
            ...
            if (SvgExtensions.IsValidStroke(svgPolyline, GeometryBounds)) {
                Stroke = SvgExtensions.GetStrokePaint(svgPolyline, GeometryBounds, AssetLoader, references,
                    IgnoreAttributes);
                if (Stroke is null) {
                    canDrawStroke = false;
                }
