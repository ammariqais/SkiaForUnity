...
namespace Svg.Model.Drawables.Elements {
    ...
    public sealed class EllipseDrawable : DrawablePath {
        ...
        public static EllipseDrawable Create(SvgEllipse svgEllipse, SKRect skViewport, DrawableBase? parent,
            ...
            drawable.IsDrawable = drawable.CanDraw(svgEllipse, drawable.IgnoreAttributes) &&
                                  drawable.HasFeatures(svgEllipse, drawable.IgnoreAttributes);
        ...
        private void Initialize(HashSet<Uri>? references) {
            ...
            if (SvgExtensions.IsValidFill(svgEllipse)) {
                Fill = SvgExtensions.GetFillPaint(svgEllipse, GeometryBounds, AssetLoader, references,
                    IgnoreAttributes);
                if (Fill is null) {
                    canDrawFill = false;
                }
            ...
            if (SvgExtensions.IsValidStroke(svgEllipse, GeometryBounds)) {
                Stroke = SvgExtensions.GetStrokePaint(svgEllipse, GeometryBounds, AssetLoader, references,
                    IgnoreAttributes);
                if (Stroke is null) {
                    canDrawStroke = false;
                }
