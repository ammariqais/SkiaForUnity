...
namespace Svg.Model.Drawables.Elements {
    ...
    public sealed class UseDrawable : DrawableBase {
        private static readonly FieldInfo? s_referencedElementParent =
            typeof(SvgElement).GetField("_parent", BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        public static UseDrawable Create(SvgUse svgUse, SKRect skViewport, DrawableBase? parent,
            ...
            drawable.IsDrawable = drawable.CanDraw(svgUse, drawable.IgnoreAttributes) &&
                                  drawable.HasFeatures(svgUse, drawable.IgnoreAttributes);
            ...
            if (width <= 0f) {
                width = new SvgUnit(SvgUnitType.Percentage, 100f).ToDeviceValue(UnitRenderingType.Horizontal, svgUse,
                    skViewport);
            }

            if (height <= 0f) {
                height = new SvgUnit(SvgUnitType.Percentage, 100f).ToDeviceValue(UnitRenderingType.Vertical, svgUse,
                    skViewport);
            }
            ...
            if (svgReferencedElement is SvgSymbol svgSymbol) {
                drawable.ReferencedDrawable = SymbolDrawable.Create(svgSymbol, x, y, width, height, skViewport,
                    drawable, assetLoader, references, ignoreAttributes);
            } else {
                var referencedDrawable = DrawableFactory.Create(svgReferencedElement, skViewport, drawable, assetLoader,
