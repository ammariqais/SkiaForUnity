...
namespace Svg.Model.Drawables.Elements {
    ...
    public sealed class RectangleDrawable : DrawablePath {
        ...
        public static RectangleDrawable Create(SvgRectangle svgRectangle, SKRect skViewport, DrawableBase? parent,
            ...
            drawable.IsDrawable = drawable.CanDraw(svgRectangle, drawable.IgnoreAttributes) &&
                                  drawable.HasFeatures(svgRectangle, drawable.IgnoreAttributes);
        ...
        private void Initialize(HashSet<Uri>? references) {
            ...
            if (SvgExtensions.IsValidFill(svgRectangle)) {
                Fill = SvgExtensions.GetFillPaint(svgRectangle, GeometryBounds, AssetLoader, references,
                    IgnoreAttributes);
                if (Fill is null) {
                    canDrawFill = false;
                }
            ...
            if (SvgExtensions.IsValidStroke(svgRectangle, GeometryBounds)) {
                Stroke = SvgExtensions.GetStrokePaint(svgRectangle, GeometryBounds, AssetLoader, references,
                    IgnoreAttributes);
                if (Stroke is null) {
                    canDrawStroke = false;
                }
