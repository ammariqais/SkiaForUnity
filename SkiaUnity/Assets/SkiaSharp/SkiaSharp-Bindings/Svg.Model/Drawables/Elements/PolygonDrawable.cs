...
namespace Svg.Model.Drawables.Elements {
    ...
    public sealed class PolygonDrawable : DrawablePath {
        ...
        public static PolygonDrawable Create(SvgPolygon svgPolygon, SKRect skViewport, DrawableBase? parent,
            ...
            drawable.IsDrawable = drawable.CanDraw(svgPolygon, drawable.IgnoreAttributes) &&
                                  drawable.HasFeatures(svgPolygon, drawable.IgnoreAttributes);
        ...
        private void Initialize(SKRect skViewport, HashSet<Uri>? references) {
            ...
            if (SvgExtensions.IsValidFill(svgPolygon)) {
                Fill = SvgExtensions.GetFillPaint(svgPolygon, GeometryBounds, AssetLoader, references,
                    IgnoreAttributes);
                if (Fill is null) {
                    canDrawFill = false;
                }
            ...
            if (SvgExtensions.IsValidStroke(svgPolygon, GeometryBounds)) {
                Stroke = SvgExtensions.GetStrokePaint(svgPolygon, GeometryBounds, AssetLoader, references,
                    IgnoreAttributes);
                if (Stroke is null) {
                    canDrawStroke = false;
                }
