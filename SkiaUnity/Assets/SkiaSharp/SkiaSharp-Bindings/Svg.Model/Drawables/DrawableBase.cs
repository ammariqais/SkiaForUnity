...
namespace Svg.Model.Drawables {
    ...
    public abstract class DrawableBase : SKDrawable, IFilterSource, IPictureSource {
        ...
        protected bool HasFeatures(SvgElement svgElement, DrawAttributes ignoreAttributes) {
            var hasRequiredFeatures = ignoreAttributes.HasFlag(DrawAttributes.RequiredFeatures) ||
                                      svgElement.HasRequiredFeatures();
            var hasRequiredExtensions = ignoreAttributes.HasFlag(DrawAttributes.RequiredExtensions) ||
                                        svgElement.HasRequiredExtensions();
            var hasSystemLanguage = ignoreAttributes.HasFlag(DrawAttributes.SystemLanguage) ||
                                    svgElement.HasSystemLanguage();
        ...
        protected bool CanDraw(SvgVisualElement svgVisualElement, DrawAttributes ignoreAttributes) {
            var isVisible = ignoreAttributes.HasFlag(DrawAttributes.Visibility) ||
                            string.Equals(svgVisualElement.Visibility, "visible", StringComparison.OrdinalIgnoreCase);
            var isDisplay = ignoreAttributes.HasFlag(DrawAttributes.Display) ||
        ...
        public virtual void Draw(SKCanvas canvas, DrawAttributes ignoreAttributes, DrawableBase? until,
        ...
        public virtual void PostProcess(SKRect? viewport, SKMatrix totalMatrix) {
            ...
            if (visualElement is { } && enableFilter) {
                var filterContext = new SvgFilterContext(visualElement, GeometryBounds, viewport ?? GeometryBounds,
                    this, AssetLoader, References);
    ...
    public virtual void DebugDrawBounds(SKCanvas canvas)
    {
        ...
        public DrawableBase? FindContainerParentBackground(DrawableBase? drawable, out SKRect skClipRect) {
            ...
            if (element.IsContainerElement() &&
                element.TryGetAttribute("enable-background", out string enableBackground)) {
