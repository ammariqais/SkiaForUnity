...
namespace Svg.Model {
    ...
    public static partial class SvgExtensions {
        ...
        internal static void CreateMarker(this SvgMarker svgMarker, SvgVisualElement pOwner, SKPoint pRefPoint,
            SKPoint pMarkerPoint1, SKPoint pMarkerPoint2, bool isStartMarker, SKRect skViewport, IMarkerHost markerHost,
            IAssetLoader assetLoader, HashSet<Uri>? references, DrawAttributes ignoreAttributes = DrawAttributes.None) {
            var fAngle1 = 0f;
            ...
            var markerDrawable = MarkerDrawable.Create(svgMarker, pOwner, pRefPoint, fAngle1, skViewport, null,
                assetLoader, references, ignoreAttributes);
            markerHost.AddMarker(markerDrawable);
        }

        internal static void CreateMarker(this SvgMarker svgMarker, SvgVisualElement pOwner, SKPoint pRefPoint,
            SKPoint pMarkerPoint1, SKPoint pMarkerPoint2, SKPoint pMarkerPoint3, SKRect skViewport,
            IMarkerHost markerHost, IAssetLoader assetLoader, HashSet<Uri>? references) {
            var xDiff = pMarkerPoint2.X - pMarkerPoint1.X;
            var yDiff = pMarkerPoint2.Y - pMarkerPoint1.Y;
            ...
            var markerDrawable = MarkerDrawable.Create(svgMarker, pOwner, pRefPoint, (fAngle1 + fAngle2) / 2,
                skViewport, null, assetLoader, references);
            markerHost.AddMarker(markerDrawable);
        }

        internal static void CreateMarkers(this SvgMarkerElement svgMarkerElement, SKPath skPath, SKRect skViewport,
            IMarkerHost markerHost, IAssetLoader assetLoader, HashSet<Uri>? references) {
            var pathTypes = skPath.GetPathTypes();
            var pathLength = pathTypes.Count;

            var markerStart = svgMarkerElement.MarkerStart;
            if (markerStart is { } && pathLength > 0 &&
                !HasRecursiveReference(svgMarkerElement, (e) => e.MarkerStart, new HashSet<Uri>())) {
                var marker = GetReference<SvgMarker>(svgMarkerElement, markerStart);
                if (marker is { }) {
                    var refPoint1 = pathTypes[0].Point;
                    var index = 1;
                    // ReSharper disable CompareOfFloatsByEqualityOperator
                    while (index < pathLength && pathTypes[index].Point.X == refPoint1.X &&
                    ...
                    CreateMarker(marker, svgMarkerElement, refPoint1, refPoint1, refPoint2, true, skViewport,
                        markerHost, assetLoader, references);
                }
            }

            var markerMid = svgMarkerElement.MarkerMid;
            if (markerMid is { } && pathLength > 0 &&
                !HasRecursiveReference(svgMarkerElement, (e) => e.MarkerMid, new HashSet<Uri>())) {
                ...
                if (marker is { }) {
                    ...
                    for (var i = 1; i <= pathLength - 2; i++) {
                        ...
                        if (bezierIndex == -1 || bezierIndex == 2) {
                            CreateMarker(marker, svgMarkerElement, pathTypes[i].Point, pathTypes[i - 1].Point,
                                pathTypes[i].Point, pathTypes[i + 1].Point, skViewport, markerHost, assetLoader,
                                references);
                        }
                    }
                }
            ...
            if (markerEnd is { } && pathLength > 0 &&
                !HasRecursiveReference(svgMarkerElement, (e) => e.MarkerEnd, new HashSet<Uri>())) {
                ...
                if (marker is { }) {
                    ...
                    if (pathLength > 1) {
                        --index;
                        // ReSharper disable CompareOfFloatsByEqualityOperator
                        while (index > 0 && pathTypes[index].Point.X == refPoint1.X &&
                               pathTypes[index].Point.Y == refPoint1.Y) {
                            --index;
                        }
                        // ReSharper restore CompareOfFloatsByEqualityOperator
                    ...
                    var refPoint2 = pathLength == 1 ? refPoint1 : pathTypes[index].Point;
                    CreateMarker(marker, svgMarkerElement, refPoint1, refPoint2, pathTypes[pathLength - 1].Point, false,
                        skViewport, markerHost, assetLoader, references);
                }
            }
        }
    }
